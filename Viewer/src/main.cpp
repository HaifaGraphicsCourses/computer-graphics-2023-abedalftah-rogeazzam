	...
	// 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
	{
		...
		if (transWindow) {
			...
			if (moreFeatures && modelIndex != -1) {
				...
				ImGui::Checkbox("Local Bound Box", &model.localBoundingBox);
				ImGui::Checkbox("World Bound Box", &model.worldBoundingBox);
		...
		if (cameraWindow) {
			...
			if (lookatCB) {
				ImGui::SliderFloat("Eye X Value", &scene.GetActiveCamera().eye.x, -360, 360);
				ImGui::SliderFloat("Eye Y Value", &scene.GetActiveCamera().eye.x, -360, 360);
				ImGui::SliderFloat("Eye Z Value", &scene.GetActiveCamera().eye.x, -360, 360);
			}
		}
